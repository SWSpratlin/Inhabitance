...
class Box {
    ...
    boolean contains(float x, float y) {
        Vec2worldPoint = box2d.coordPixelsToWorld(x, y);
    ...
    void display() {
        ...
        Vec2pos = box2d.getBodyPixelCoord(body);
    ...
    //method to return the YPosition of the body
    //used mainly for the amplitude control of sound
    float getPixelCoordY() {
        Vec2yPos = box2d.getBodyPixelCoord(body);
        return(yPos.y);
    }
    ...
    void makeStop(float t_) {
        ...
        Vec2oVel = new Vec2(body.getLinearVelocity()); //pull linear velocity from box
        ...
        Vec2stepVec = new Vec2(stepX / (t / 15),stepY / (t / 15)); //step vector for subtraction purposes
